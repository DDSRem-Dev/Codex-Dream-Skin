...
MIZUKI_CHECK="$("$NODE" "$ROOT/scripts/injector.mjs" --check-payload --theme-dir "$MIZUKI_THEME")"
"$NODE" -e '
  const value = JSON.parse(process.argv[1]);
  if (!value.pass || value.themeId !== "mizuki-25ji" || value.imageBytes < 1) process.exit(1);
...
# Bundled macOS themes are refreshed without touching user custom-* packs.
/usr/bin/env HOME="$TMP/seed-home" /bin/bash -c '
  ...
  seed_bundled_themes
  seed_bundled_themes
  /usr/bin/cmp -s "$1/themes/mizuki-25ji/theme.json" "$themes/mizuki-25ji/theme.json"
  /usr/bin/cmp -s "$1/themes/mizuki-25ji/mizuki-25ji.jpg" "$themes/mizuki-25ji/mizuki-25ji.jpg"
  /usr/bin/cmp -s "$1/themes/mizuki-25ji/mizuki-official-now.webp" "$themes/mizuki-25ji/mizuki-official-now.webp"
...
if [ "${CODEX_DREAM_SKIN_SKIP_SIGNED_RUNTIME_TESTS:-0}" = "1" ]; then
  printf 'SKIP: switch-theme integration requires an installed, signed ChatGPT app.\n'
...
"$NODE" "$ROOT/scripts/injector.mjs" --watch --port 19341 --theme-dir "$ROOT/themes/mizuki-25ji" \
...
if [ "${CODEX_DREAM_SKIN_SKIP_SIGNED_RUNTIME_TESTS:-0}" = "1" ]; then
  printf 'SKIP: runtime-state integration requires an installed, signed ChatGPT app.\n'
...
"$NODE" -e '
  ...
  if (theme.colors?.accent !== "#11aa55" || theme.colors?.secondary !== "#22bbcc") process.exit(1);
  if (theme.colors?.highlight !== "#663399" || theme.appearance !== undefined || theme.art !== undefined) process.exit(1);
...
"$NODE" -e '
  ...
  fs.writeFileSync(process.argv[1], `${JSON.stringify({
    ...
    appearance: "dark",
    art: { focusX: 0.12, focusY: 0.88, safeArea: "none", taskMode: "off" },
  }, null, 2)}\n`);
' "$TMP/explicit-theme/theme.json"
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.2.7" ]' _ "$ROOT"
...
printf 'PASS: syntax, payload, bundled themes, runtime-state safety, custom-theme, config round-trips, HOME recovery; switch-theme %s, runtime-state integration %s, %s.\n' \
